...
def Gaussian():
    ...
    # Tính bộ lọc
        for u in range(U):
            for v in range(V):
                if  np.exp((-D[np.abs(u - U0), np.abs(v - V0)]**2)/(2*(D0**2))):
                    H[u, v] = 1
                else:
                    H[u, v] = 0
        return H
        
    H_uv = lowPass_Gaussian(60,P,Q)
    #B5.2: Nhân ảnh sau khi DFT với ảnh sau khi lọc
    ax5.imshow(H_uv, cmap='gray')
    ax5.set_title('Bước 5.1: H(u,v) đối xứng qua tâm bộ lọc ')
    ax5.axis('off')
...
def Butterworth():
    ...
    ax4.axis('off')
    #B5.1: Cho hàm lọc có giá trị thực H(u,v) đối xứng qua tâm
    def lowPass_butterworth(D0,U,V,n):
        # H is our filter
        H = np.zeros((U, V))
        D = np.zeros((U, V))
        U0 = int(U / 2)
    ...
    # Tính bộ lọc
        for u in range(U):
            for v in range(V):
                if  1/((1+ D[np.abs(u - U0), np.abs(v - V0)]/D0)**(2*n)):
                    H[u, v] = 1
                else:
                    H[u, v] = 0
        return H
    H_uv = lowPass_butterworth(60,P,Q,2)
    #B5.2: Nhân ảnh sau khi DFT với ảnh sau khi lọc
    ax5.imshow(H_uv, cmap='gray')
    ax5.set_title('Bước 5.1: H(u,v) đối xứng qua tâm bộ lọc ')
    ax5.axis('off')
...
window = Tk()
window.title("Biến đổi fourier thử nghiệm phân tích phổ của ảnh và ứng dụng trong xử lí lọc nhiễu đa mức xám")
window.geometry("1366x768")

label = Label(window,text="BÀI THỰC HÀNH MÔN XỬ LÍ ẢNH " ,font=("Arial",20))
label.pack(side=TOP,pady=5 )

frm = Frame(window)
frm.pack(side=TOP , padx= 5 , pady= 15)

lbl = Label(window)
lbl.pack()

btn1 = Button(frm,text="Chọn Ảnh",command=ChonAnh ,background = "red", fg = "white")
btn1.pack(side=TOP,padx=5 ,pady=5,expand=True ,fill=BOTH )

btn2 = Button(frm,text="Lọc Thông Thấp Ideal",command=LowpassIdeal ,background = "red", fg = "white")
btn2.pack(side=TOP,padx=5, pady=5,expand=True ,fill=BOTH) 

btn3 = Button(frm,text="Lọc Thông Thấp GauSsian",command=Gaussian,background = "red", fg = "white")
btn3.pack(side=TOP,padx=5 , pady=5,expand=True ,fill=BOTH) 

btn4 = Button(frm,text="Lọc Thông Thấp ButterWorth", command=Butterworth,background = "red", fg = "white")
btn4.pack(side=TOP,padx=5, pady=5,expand=True ,fill=BOTH) 
